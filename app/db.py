...
async def get_email_notification_stats(dept: str | None = None, ug_or_pg: str | None = None) -> list[dict]:
    db = get_db()
    query: dict[str, Any] = {}
    if dept:
        query["program"] = dept
    ...
    async for u in db[USERS].find(query):
        dept_name = u.get("program") or "No Program"
        if dept_name not in results:
            results[dept_name] = {
                ...
                "clicked": 0,
                "completed_after": 0,
                "clicked_not_submitted": 0,
                "in_draft": 0,
                "total_users": 0
            }

        entry = results[dept_name]
        entry["total_users"] += 1
        ...
        elif clicked and (u.get("status") or "not_started") != STATUS_POST_DONE:
            # Clicked the reminder link but still hasn't submitted → follow up.
            entry["clicked_not_submitted"] += 1
